...
- `backend`、`notification`、`core-control`、`web` 不再在服务器本地 `build`。启动时先由 `release-sync` 容器优先读取宿主机 `github-release/` 缓存，缺失时再从 GitHub Release 下载 `backend_bundle.tar.gz` 和 `web_bundle.tar.gz` 到 `github-release/`，随后解压到 `deploy/release`。
