...
GH_RELEASE_PROXY_PREFIX=https://gh-proxy.org # 默认已启用，可改
GH_RELEASE_DOWNLOAD_BASE= # 可选，覆盖默认下载地址
...
- `GH_RELEASE_PROXY_PREFIX`：给 GitHub Release 直链下载加代理前缀（默认 `https://gh-proxy.org`）
- `GH_RELEASE_DOWNLOAD_BASE`：覆盖默认下载基址（默认 `https://github.com/<repo>/releases/download/<tag>`）
